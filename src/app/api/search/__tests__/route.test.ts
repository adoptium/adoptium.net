...
vi.mock("@/services/asciidocService", () => ({
  getAllAsciidocPaths: vi.fn(),
  getAsciidocContent: vi.fn(),
}));

const mockGetAllAsciidocPaths =
  asciidocService.getAllAsciidocPaths as unknown as ReturnType<typeof vi.fn>;
const mockGetAsciidocContent =
  asciidocService.getAsciidocContent as unknown as ReturnType<typeof vi.fn>;
...
describe("GET /api/search", () => {
  ...
  it("returns matching results", async () => {
    ...
    mockGetAsciidocContent.mockImplementation(async (slug) => {
      if (slug === "foo")
        return { metadata: { title: "Foo Title", description: "Some desc" } };
      if (slug === "bar")
        return { metadata: { title: "Bar", description: "Other desc" } };
      return null;
    });
  ...
  it("returns empty array if no matches", async () => {
    mockGetAllAsciidocPaths.mockResolvedValue([{ slug: "foo", locale: "en" }]);
    mockGetAsciidocContent.mockResolvedValue({
      metadata: { title: "Nope", description: "Nothing" },
    });
