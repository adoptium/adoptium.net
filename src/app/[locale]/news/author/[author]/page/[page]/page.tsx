...
export default async function AuthorNewsPagePaginated({
  ...
  const { posts, totalPages } = await getNewsByAuthor(author, {
    numPosts: 6,
    page: pageNumber,
  });
  ...
  if (authorData.twitter)
    socialLinks.push(`https://x.com/${authorData.twitter}`);
  if (authorData.github)
    socialLinks.push(`https://github.com/${authorData.github}`);
  if (authorData.linkedin)
    socialLinks.push(`https://www.linkedin.com/in/${authorData.linkedin}`);
  ...
  return (
    <div>
      ...
      <NewsCardList
        ...
        previousPageLink={
          pageNumber > 2
            ? `/news/author/${author}/page/${pageNumber - 1}`
            : `/news/author/${author}`
        }
        nextPage={
