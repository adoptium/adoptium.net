...
export default async function TemurinNightlyPage() {
  ...
  const allVersions = (availableReleases.available_releases ?? []).map(
    (version: number) => {
      const isLTS = (availableReleases.available_lts_releases ?? []).includes(
        version,
      );
      return {
  ...
  if (
    !allVersions.some((v) => v.value === mostRecentFeatureVersion.toString())
  ) {
    allVersions.unshift({
      name: `${mostRecentFeatureVersion} - EA`,
      value: mostRecentFeatureVersion.toString(),
    });
  }
  const latestLTS = availableReleases.most_recent_lts ?? 21;

  return (
    <TemurinNightlyPageClient allVersions={allVersions} latestLTS={latestLTS} />
  );
}
