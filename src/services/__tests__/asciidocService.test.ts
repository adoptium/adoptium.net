...
import { getAsciidocContent, getAllAsciidocPaths, getPagesInSection } from '../asciidocService';
...
This is test content.`;
    ...
    it('should get content for a simple slug with default locale', async () => {
      const slug = 'about';
      const expectedPath = path.join(process.cwd(), 'content/asciidoc-pages/about/index.en.adoc');
      const fallbackPath = path.join(process.cwd(), 'content/asciidoc-pages/about/index.adoc');
      ...
      expect(mockProcessAsciiDoc).toHaveBeenCalledWith(fallbackPath, mockContent);
      expect(mockExtractMetadata).toHaveBeenCalledWith(fallbackPath, mockContent);
      expect(mockGetLanguageVariants).toHaveBeenCalledWith(fallbackPath, 'index');
    ...
    it('should get content for a specific locale', async () => {
      const slug = 'about';
      const locale = 'zh-CN';
      const expectedPath = path.join(process.cwd(), 'content/asciidoc-pages/about/index.zh-CN.adoc');
    ...
    it('should handle nested slugs', async () => {
      ...
      const expectedPath = path.join(process.cwd(), 'content/asciidoc-pages/docs/installation/guide/index.en.adoc');
      const fallbackPath = path.join(process.cwd(), 'content/asciidoc-pages/docs/installation/guide/index.adoc');
    ...
    it('should filter empty slug parts', async () => {
      ...
      const expectedPath = path.join(process.cwd(), 'content/asciidoc-pages/docs/installation/index.en.adoc');
    ...
    it('should handle empty slug', async () => {
      const slug = '';
      const expectedPath = path.join(process.cwd(), 'content/asciidoc-pages/index.en.adoc');
  ...
  describe('getAllAsciidocPaths', () => {
    it('should traverse directories and find all AsciiDoc files', async () => {
      ...
      const mockAboutEntries: MockDirent[] = [
        { name: 'index.adoc', isDirectory: () => false, isFile: () => true },
        { name: 'index.zh-CN.adoc', isDirectory: () => false, isFile: () => true },
      ];
    ...
    it('should handle complex locale patterns', async () => {
      const mockDirEntries: MockDirent[] = [
        { name: 'index.adoc', isDirectory: () => false, isFile: () => true },
        { name: 'index.en-US.adoc', isDirectory: () => false, isFile: () => true },
        { name: 'index.pt-BR.adoc', isDirectory: () => false, isFile: () => true },
        { name: 'index.zh_CN.adoc', isDirectory: () => false, isFile: () => true },
    ...
    it('should filter pages in a specific section with preferred locale', async () => {
      ...
      const createMockFileSystem = () => {
        ...
        const mockDocsEntries: MockDirent[] = [
          { name: 'index.adoc', isDirectory: () => false, isFile: () => true },
          { name: 'installation', isDirectory: () => true, isFile: () => false },
          { name: 'configuration', isDirectory: () => true, isFile: () => false },
        ...
        const mockConfigurationEntries: MockDirent[] = [
          { name: 'index.adoc', isDirectory: () => false, isFile: () => true },
          { name: 'index.fr.adoc', isDirectory: () => false, isFile: () => true },
        ];
        ...
        (mockFs.readdirSync as unknown as Mock).mockImplementation(() => {
          switch (callCount++) {
            case 0: return mockEntries;
            case 1: return mockDocsEntries;
            case 2: return mockInstallationEntries;
            case 3: return mockConfigurationEntries;
            case 4: return mockGuidesEntries;
            case 5: return mockSetupEntries;
            case 6: return mockAboutEntries;
            case 7: return mockSupportEntries;
            default: return [];
          }
        });
      };
      ...
      expect(result.every(page => page.slug.startsWith('docs') && page.slug !== 'docs')).toBe(true);
    });
  ...
  describe('edge cases and error handling', () => {
    it('should handle malformed file names gracefully', async () => {
      const mockEntries: MockDirent[] = [
        { name: 'index.adoc', isDirectory: () => false, isFile: () => true },
        { name: 'malformed.file.name.adoc', isDirectory: () => false, isFile: () => true },
        { name: 'index.invalid-locale.adoc', isDirectory: () => false, isFile: () => true },
        { name: 'not-index.adoc', isDirectory: () => false, isFile: () => true },
      ];
    ...
    it('should handle very deep directory structures', async () => {
      ...
      const expectedPath = path.join(process.cwd(), 'content/asciidoc-pages/very/deep/nested/path/structure/index.en.adoc');
