...
const splitDrilldownSeriesByArtifacts = (data: DrilldownEntry[]) => {
  const mArtifacts = new Map<
    string,
    { name: string; id: string; data: DrilldownEntry[] }
  >();
  data.forEach((entry) => {
    const { artifact, name } = getArtifactAndFormattedName(entry);
    if (artifact !== undefined && name !== undefined) {
      entry.name = name;
      if (!mArtifacts.has(artifact))
        mArtifacts.set(artifact, { name: artifact, id: artifact, data: [] });
...
const getArtifactAndFormattedName = (entry: { name: string }) => {
  ...
  if (tokens.length > 0 && tokens[0].endsWith("sources")) {
    ...
    const name =
      entry.name.endsWith(".msi") || entry.name.endsWith(".pkg")
...
const ColumnDrilldown: React.FC<ColumnDrilldownProps> = ({
  name,
  availableReleases,
}) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      import("highcharts/modules/drilldown").then((mod) => {
        const drilldown = mod as unknown as
          | { default?: (h: typeof Highcharts) => void }
          | ((h: typeof Highcharts) => void);
        if (typeof drilldown === "function") {
          (drilldown as (h: typeof Highcharts) => void)(Highcharts);
        } else if (
          typeof (drilldown as { default?: (h: typeof Highcharts) => void })
            .default === "function"
        ) {
          (drilldown as { default: (h: typeof Highcharts) => void }).default(
            Highcharts,
          );
        }
      });
  ...
  useEffect(() => {
    const updateData = async () => {
      ...
      const seriesDataPromises = availableReleases.available_releases.map(
        async (key) => {
          const apiData: Record<string, number> | null =
            await fetchDownloadStats(key);
          if (!apiData) return null;
          const drilldownDataPromises = Object.keys(apiData).map(
            async (apiDataKey) => {
              const secondLevelApiData = await fetchDownloadStats(
                key,
                apiDataKey,
              );
              if (!secondLevelApiData) return null;
              const secondLevelDrilldownSeriesData: DrilldownEntry[] =
                Object.keys(secondLevelApiData).map((secondLevelApiKey) => ({
                  name: secondLevelApiKey,
                  y: secondLevelApiData[secondLevelApiKey],
                  drilldown: secondLevelApiKey,
                }));
              const r = splitDrilldownSeriesByArtifacts(
                secondLevelDrilldownSeriesData,
              );
              archLevelDrilldownSeries.push({
                name: apiDataKey,
                id: apiDataKey,
                data: Object.keys(r.artifacts)
                  .sort((a, b) => a.localeCompare(b))
                  .map((val) => ({
                    name: val,
                    y: r.artifacts[val].data.reduce(
                      (a: number, b: DrilldownEntry) => a + b.y || 0,
                      0,
                    ),
              ...
              Object.keys(r.artifacts).forEach((val) => {
                archLevelDrilldownSeries.push({
                  name: `${apiDataKey}-${val}`,
                  id: `${apiDataKey}-${val}`,
                  data: r.artifacts[val].data.sort(
                    (a: DrilldownEntry, b: DrilldownEntry) =>
              ...
              return {
                name: apiDataKey,
                y: secondLevelDrilldownSeriesData.reduce(
                  (a, b) => a + b.y || 0,
                  0,
                ),
          ...
          return {
            name: `JDK${key}`,
            y: (drilldownSeriesData as DrilldownEntry[]).reduce(
              (a, b) => a + b.y || 0,
              0,
            ),
            drilldown: key,
      ...
      const seriesData = (await Promise.all(seriesDataPromises)).filter(
        Boolean,
      ) as DrilldownEntry[];
