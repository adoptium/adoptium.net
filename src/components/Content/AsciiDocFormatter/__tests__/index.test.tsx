...
vi.mock("@/components/Content/Asciidoc/AccordionItem", () => ({
  __esModule: true,
  default: ({
    title,
    children,
...
describe("AsciiDocFormatter", () => {
  ...
  it("transforms table of contents correctly", () => {
    ...
    // TOC is hidden (rendered in sidebar instead) so section links should not appear
  ...
  it("transforms iframe tags correctly", () => {
    const content =
      '<iframe src="https://example.com/embed" title="Sample Embed"></iframe>';
    const { container } = render(<AsciiDocFormatter content={content} />);
  ...
  it("transforms icon td cells correctly", () => {
    const content =
      '<table><tbody><tr><td class="icon"></td></tr></tbody></table>';
    const { container } = render(<AsciiDocFormatter content={content} />);

    // Icon td cells are not individually transformed; the admonition block transform handles them
