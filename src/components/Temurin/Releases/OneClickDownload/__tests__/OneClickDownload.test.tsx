...
describe("OneClickDownload component", () => {
  ...
  it("should render macOS and Windows download cards with default versions", () => {
    ...
    expect(
      screen.getByLabelText("Download Windows package"),
    ).toBeInTheDocument();
  });
  ...
  it("should prioritize macOS ARM over x64", () => {
    ...
    expect(
      screen.getByText("Temurin 11.0.28+6, macOS aarch64 (M1) (.PKG)"),
    ).toBeInTheDocument();
  });
  ...
  it("should fall back to x64 when ARM is not available", () => {
    ...
    expect(
      screen.getByText("Temurin 11.0.27+6, macOS x64 (.PKG)"),
    ).toBeInTheDocument();
  });
  ...
  it("should prefer installer links over archive links", () => {
    ...
    expect(
      screen.getByText("Temurin 11.0.28+6, macOS aarch64 (M1) (.TAR.GZ)"),
    ).toBeInTheDocument();
    // Windows should show installer format
    expect(
      screen.getByText("Temurin 11.0.28+6, Windows 64 bit (.MSI)"),
    ).toBeInTheDocument();
  });
  ...
  it("should show different versions for different platforms", () => {
    ...
    expect(
      screen.getByText("Temurin 11.0.27+6, macOS aarch64 (M1) (.PKG)"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Temurin 11.0.28+6, Windows 64 bit (.MSI)"),
    ).toBeInTheDocument();
    ...
    const releases: BinaryAssetView[] = [
      {
        ...
        // no binary properties -> component finds no mac/windows matches
      },
    ];
  ...
  it("should handle selectedVersion fallback when first release is undefined", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "macOS" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: "Windows" }),
    ).not.toBeInTheDocument();
  });
});
