...
const ReleaseResults: React.FC<ReleaseResultsProps> = ({
  ...
  const groupedReleases = releases.reduce<
    Record<string, Record<string, ReleaseAsset[]>>
  >((acc, release) => {
    const { os } = release;
    if (!acc[os]) {
      acc[os] = {};
    ...
    <section className="py-4" data-testid="release-results-content">
      <div className="space-y-4">
        {sortedOSEntries.map(([os, architectures]) => {
          ...
          // Sort architecture keys anti-alphabetically (reverse alphabetical)
          const architectureList = Object.keys(architectures).sort((a, b) =>
            b.localeCompare(a)
          );

          // Default: x64 → if not present, fall back to first in sorted list
          const selectedArch =
          ...
          return (
            ...
            >
                        ...
                        {architectureList.map((arch) => (
                          ...
                          >
                            <span
                              ...
                              outline-hidden cursor-pointer transition-all duration-200 ease-in-out ${
                                selectedArch === arch
              ...
              <div className="flex flex-col w-full lg:w-[50%] mt-8 lg:mt-0">
                ...
                >
                  {`Temurin ${releaseGroup[0]?.release_name || ""} - ${
                    releaseGroup[0]?.release_date
                      ? new Date(
                          releaseGroup[0].release_date
                  ...
                  release.binaries
                    .filter(
                      (binary) =>
                        ...
                        binary.type.toLowerCase() ===
                          getPackageType(os, selectedArch).toLowerCase()
                    )
                    .map((binary, idx) => (
                      <div
                        ...
                        <div className="flex justify-between w-full items-center">
                          <div className="flex items-center gap-2">
                            ...
                            <h5
                              ...
                              data-testid={`binary-info-${release.os}-${release.architecture}-${binary.type}-${idx}`}
                            >
                      ...
                      >
                        <div className="flex justify-between w-full items-center">
                          <div className="flex items-center gap-2">
                            ...
                            <h5 className="text-base font-normal">
                              {`${
                                binary.installer_extension?.toUpperCase() ||
                                "INSTALLER"
                              }, ${binary.installer_size} MB`}
                            </h5>
