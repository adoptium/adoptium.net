 ...
 */
async function getMaxContributors(
  repository: string,
...
async function getContributor(
  ...
  return {
    ...
    repo: repository,
    contributionsCount: contributor.contributions ?? 0,
...
async function fetchRandomContributor() {
  ...
  try {
    ...
    if (!maxContributors || needToRefetch) {
      // get fresh data
      const [randomPageUpdate, lastPageUpdate] =
        await getMaxContributors(repository);
    ...
    while (
      ...
      randomPage =
        Math.floor(Math.random() * Math.floor(maxContributors || 1)) + 1;
