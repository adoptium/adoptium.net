...
date: "2026-03-30"
author: pmc
description: "The Adoptium PMC highlights notable technical achievements with large positive impact on the project, including infrastructure optimisations, release delivery time improvements, and package hardening achieved in the final quarter of 2025."
tags:
  - Adoptium
  - announcement
  - contributors
...
- **Key Contributors**: Antonio Vieiro and David Sladky, with guidance from Andrew Leonard.
...
- **Engineering Optimisation**: Sophia Guo and Andrew Leonard implemented logic in the AQAvit test framework to rerun only failed arctic test cases rather than entire test targets. This work could then be leveraged by the Temurin Compliance project to significantly lower infrastructure use and compute costs and also reduce the time required to verify that Temurin builds are TCK compliant. Issue: [Extend the 'Rerun failed testcases' feature to arctic targets aqa-tests#6756](https://github.com/adoptium/aqa-tests/issues/6756), PRs: [Parsing regular expression for arctic TKG#779](https://github.com/adoptium/TKG/pull/779) and [Extended the rerun failed testcases feature to arctic aqa-tests#6757](https://github.com/adoptium/aqa-tests/pull/6757) and [Update Arctic runtests.sh to handle multiple ARCTIC_GROUPS for "custom" aqa-tests#6818](https://github.com/adoptium/aqa-tests/pull/6818)
...
- **Scaling**: Stewart Addison and George Adams enabled AQA testing in containers on Linux/x64 dynamically provisioned Azure VMs. PR: [Utilise containers in Azure dynamic VMs for AQA runs aqa-tests#6553](https://github.com/adoptium/aqa-tests/pull/6553).
...
The PMC continues to prioritise the integrity and predictability of our installers. In Q4, Scott Fryer addressed a non-determinism issue in the Temurin Debian packaging process. Issue: [jpackage works on Zulu but not on Temurin installer#1274](https://github.com/adoptium/installer/issues/1274) PR: [Fix Temurin Debian packages to be non-deterministic. installer#1277](https://github.com/adoptium/installer/pull/1277)
...
While the 2025 Q4 contributions of Antonio, David, Andrew, Sophia, Scott, Stewart, and George have been highlighted, we thank the entire community for their commitment to the technical excellence of Eclipse Adoptium and look forward to celebrating many future achievements.
