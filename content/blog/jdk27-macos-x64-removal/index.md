...
- **The upstream port is no longer maintained.** The OpenJDK macOS x64 port itself is no longer actively maintained upstream. Continuing to ship a build on a platform that is no longer supported by the wider ecosystem is not something we can responsibly do.
