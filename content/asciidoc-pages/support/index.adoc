...
:col-first-availability: First Availability
:col-latest-release: Latest Release
:col-next-release-due: Next Release Due
:col-end-of-availability: End of Availability
// Month abbreviations
:month-jan: Jan
:month-feb: Feb
:month-mar: Mar
...
When OpenJDK chooses to produce a CSPU release in order to address security
issues, Adoptium will also produce a release. These will typically take
place on the third Tuesday of a month that does not already contain a
release. Expected CSPUs are listed [here](https://ops.java/releases/), where
the version strings have four numbers each (e.g. 25.0.5.1) instead of three.
...
weeks. We always prioritise the most popular platforms which will
typically appear within a few days of these dates.
