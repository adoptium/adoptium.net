...
[IMPORTANT]
====
Repository URLs under `https://packages.adoptium.net/artifactory/` are intended for package manager configuration (`apt`, `dnf`, `yum`, `zypper`, `apk`) and may return `404` when opened directly in a browser. To browse available package trees manually, use the `https://packages.adoptium.net/ui/native/` links shown in each section below.
====
