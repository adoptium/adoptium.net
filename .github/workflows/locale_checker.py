...
def build_issue_body(english_file, english_shasum, outdated_locales, root):
    ...
    for locale in outdated_locales:
        ...
        url = f"https://github.com/{REPO}" f"/blob/main/{loc_path}"
        body += f"| {locale.upper()} " f"| [{loc_file}]({url}) " f"| {locale_lead}\n"
    body += "\n"
    body += "```diff\n"
    ...
    for locale in outdated_locales:
        ...
        body += "```diff\n"
        body += os.popen(
            f"git diff {localized_shasum}..{english_shasum}" f" -- {english_file}"
        ).read()
        body += "```\n\n"
        body += "</details>\n\n"
    return body
    ...
    for root, _, files in os.walk(CONTENT_DIR):
        ...
        check_issue = (
            os.popen(
                "gh issue list "
                + f"-R {REPO} "
                + "--search 'Translation review required"
                + " after updates to "
        ...
        if check_issue:
            print(f"An issue already exists for " f"{english_file}: {check_issue}")
            continue
